...
public class MyAgent3 {
	...
	MyAgent3(int maxDepth, double leftTime, double timeOneStep) {
		// this.root = root;
	...
	private Result minimaxValue(Node node, double alpha, double beta) {
		...
		for (int[] moveCoor : sortedFruitBlocks.keySet()) {
			// sort fruit blocks in decreasing order of block sizes
			// Map<int[], Integer> sortedFruitBlocks = sortFruitBlocks(node.boardState.getFruitBlocks());
			// choose one legal action
			// List<int[]> chosenBlock = sortedFruitBlocks.get(i);
			// apply action to node
			// int[] moveCoor = new int[]{chosenBlock.get(0)[0], chosenBlock.get(0)[1]};
			 // System.out.println("movecoor: [" + moveCoor[0] + ", " + moveCoor[1] + "]");
			// Node child = applyAction(node, chosenBlock);
			Node child = applyAction(node, moveCoor, sortedFruitBlocks.get(moveCoor));
			nodeCount++;
			 // System.out.println("Add Child# " + i + ": \n");
